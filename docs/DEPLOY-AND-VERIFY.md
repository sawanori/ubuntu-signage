...
> **対象 OS = Ubuntu Desktop 24.04**（Pi4: arm64 / x86 機: amd64）。  
> **手動運用**の初回セットアップ・日常操作は **README のクイックスタート**を参照してください。  
> 本書の § 5（autologin / systemd 自動起動）・AppImage / libfuse2 / Wayland（wayfire, labwc）等の節は**オプションの無人 kiosk 運用向け**で、手動運用には不要です。
...
**libfuse2 の前提インストール（AppImage 経路のみ必要・Ubuntu の deb 経路では不要）**

Ubuntu は既定で fuse3 のみ搭載しており、type-2 AppImage の実行に必要な `libfuse2` が含まれません（deb パッケージ経路では不要です）。AppImage 経路で運用する場合はインストール前に以下を実行してください:

```bash
# 実機で要実行 (Ubuntu・AppImage 経路)
...
> `videoFolderPath` には実在する絶対パスを指定してください（例: `/home/ubuntu/videos`）。
...
# E2E テスト (Playwright)
# package.json の "test:e2e" は "xvfb-run -a playwright test" を内包しているため
# npm run test:e2e だけで実行できる (xvfb-run を二重に指定しないこと)
npm run test:e2e
```
...
**実機でしか行えません** (実装計画書 §9.4 リスク一覧 / T09 実機ゲート参照)。
...
```bash
# 実機で要実行（<user>/<host> は実際のユーザー名・ホスト名に置換）
# AppImage を対象機へ転送
...
> **重要: deb パッケージ単独では自動起動は設定されません**
>
> `sudo apt install ./ubuntuapp_*.deb` は `/opt/ubuntuapp/ubuntuapp` へのランチャを置くのみで、
> 24h 無人運用に必要な **systemd user unit** (自動起動・自動再起動) と
> **XDG autostart (.desktop)** の配線を含みません。
> 無人 kiosk 運用には **AppImage + `ops/install.sh`** 経路を使ってください。
> deb パッケージは手動起動・開発用途向けです。

### 5.1 前提

**自動ログイン設定（必須）**

サイネージアプリを boot 時に自動起動するには、**デスクトップ自動ログイン** が必須です。
自動ログインなしの場合、XDG autostart は起動しません (`loginctl enable-linger` は
グラフィカルセッションを起動しないため代替になりません)。

**Ubuntu 24.04（GDM3）での設定方法:**

- **GUI**: 設定 → システム → ユーザー → 自動ログインを ON にする（`<ユーザー名>` を選択）
- **CLI** (`/etc/gdm3/custom.conf` を編集):
  ```bash
  # 実機で要実行 (Ubuntu 24.04)
  sudo nano /etc/gdm3/custom.conf
  # [daemon] セクションに以下の2行を追加:
  #   AutomaticLoginEnable=true
  #   AutomaticLogin=<ユーザー名>  ← 実際のログインユーザー名に置換
  ```
  例:
  ```ini
  [daemon]
...
設定後に再起動してください (`sudo reboot`)。

> **注意: §5 は無人 kiosk 運用専用です。担当者が朝手動起動する運用では自動ログイン設定は不要です。**
...
```bash
# プロジェクトルートで実行
# 引数: 対象機上での AppImage の絶対パス（<ユーザー名> を実際のユーザー名に置換）
bash ops/install.sh /home/<ユーザー名>/ubuntuapp-0.1.0-arm64.AppImage
```
...
```bash
# 実機で要実行（/home/<ユーザー名>/ は実際のホームディレクトリに置換）
...
sed -i "s|ExecStart=.*|ExecStart=$HOME/ubuntuapp-0.1.0-arm64.AppImage|" \
  ~/.config/systemd/user/signage-overlay.service
...
| `Environment=XDG_SESSION_TYPE=x11` | `x11` (プレースホルダ) | install.sh が実行環境の値 (x11 または wayland) に上書き。アプリの isWayland 判定に使用 |
...
**注意: Pi OS Bookworm 既定セッション (Wayfire/labwc) は Wayland です。以下は X11 セッション専用です。**

X11 セッションでは `powerSaveBlocker` に加えて `xset` コマンドで OS レベルのスリープを抑止します。  
アプリが起動時に自動実行しますが、手動でも確認できます:

```bash
# 実機で要実行 (X11 セッション上で。Wayland では xset は無効)
...
### 6.3 Wayland 環境での補完設定 (Pi OS Bookworm 既定)

Pi OS Bookworm は既定で **Wayfire または labwc** (Wayland コンポジタ) を使用します。  
Wayland では `xset` は動作しません。スリープ抑止の主防御は §6.1 の `powerSaveBlocker` です。

**コンポジタ別フォールバック (要確認):**

| コンポジタ | 対応方法 |
|-----------|---------|
| Wayfire | `~/.config/wayfire.ini` の `[idle]` プラグインを無効化 (`[idle]\nenabled=false`) **[要実機確認]** |
| labwc | swayidle が未導入であれば追加設定は不要な場合が多い **[要実機確認]** |

**GNOME + Wayland の場合 (Pi OS 標準ではない):**

```bash
# GNOME デスクトップの場合のみ有効 (wayfire/labwc では gsettings は効かない)
...
```bash
# consoleblank=0 はテキスト仮想端末 (VT) のブランキング抑止。
# グラフィカルデスクトップ (Wayland/X11) のスリープとは別物であり、本アプリには関係なし。
# /boot/cmdline.txt への追加は不要。
```

> 実機で 30 分以上放置し、スリープが発動しないことを CHK-10 で確認してください。
> `powerSaveBlocker` が Wayland idle-inhibit API 経由で正常に動作していれば追加設定なしでスリープは抑止されます。

### 6.4 Pi4 の電源設定 (lightdm/X11 サーバ用・Pi OS Bookworm 既定では非該当)

**注意: 以下は lightdm + X11 サーバ構成専用です。Pi OS Bookworm 既定の Wayfire/labwc には該当しません。**

```bash
# 実機で要実行 (lightdm 使用時のみ)
grep -r "xserver-command" /etc/lightdm/ || echo "設定なし"

# lightdm.conf の [SeatDefaults] セクションに以下を追加 (lightdm 使用時のみ):
...
# CPU 温度（Ubuntu 標準・vcgencmd 不要）
awk '{printf "%.1f°C\n",$1/1000}' /sys/class/thermal/thermal_zone0/temp 2>/dev/null || echo "N/A"
# Pi4 固有の GPU 温度が必要な場合: sudo apt install libraspberrypi-bin（既定未導入）してから vcgencmd measure_temp
```

**期待結果 (Pi4 受け入れ閾値):**
- フレームレート: 30fps 以上を維持
- CPU 使用率: 4 コア合計で 80% 以下
- 温度: 80°C 以下（`/sys/class/thermal/thermal_zone0/temp` または `vcgencmd measure_temp`）
- プロセス合計メモリ: 512MB 以内

HW デコードが使えない場合はソフトデコードにフォールバックして継続動作します。  
結果を `docs/pi4-verification.md` に記録してください。

> **注意 (Pi4 固有):** Pi4 は VideoCore VI / V4L2 stateful decoder を搭載しており、
> アプリが起動時に付与する `--enable-features=VaapiVideoDecoder` が
> Pi4 実機で engage するとは限りません。
> 実機で `vainfo` コマンドと CPU/fps 実測で効果を確認し、
> 効かない場合は V4L2 系フラグ (`--enable-features=VaapiVideoDecoder,VaapiVideoDecodeLinuxGL` 等) や
> Pi 固有の手法を別途検討してください (実機判定事項)。
...
2. ログにイベント `input.shortcutSkipped` (reason: wayland, accelerator: Control+G) の記録があることを確認する
   ```bash
   journalctl --user -u signage-overlay | grep input.shortcutSkipped
   ```
...
**隅 3 タップの誤発火確認 (CHK-14 統合):**
- 1〜2 回タップでは設定パネルが開かないことを確認する (誤発火なし)
- 再度 3 タップでパネルが閉じることを確認する
...
対応テスト: pi4-verification.md CHK-12

**手順:** 広告動画再生中に下層サイネージ上でマウスをクリック・ドラッグする。

**期待結果:**
- 動画再生中のマウス操作が下層サイトに届く
- overlayView に `pointer-events: none` が適用されているため、動画上のクリックがサイネージ側に透過する

---

### [CHK-13] アドレスバー (Phase E)

対応テスト: pi4-verification.md CHK-13

**手順:** 画面上部中央ゾーンをタップしてアドレスバーを表示し、URL を入力して遷移確認。

**期待結果:**
- 上部中央ゾーンのタップでアドレスバーが表示される
- URL 入力後 Enter でナビゲートが実行される
- 許可外 URL が拒否される
- バー外クリックでアドレスバーが非表示になる

---

*このドキュメントのコマンドはすべて実際の npm スクリプト (`package.json` の `scripts` セクション) または標準 Linux コマンドを使用しています。存在しないスクリプトは参照していません。*

*チェックリスト項目数: 13 項目 (CHK-01 〜 CHK-13、CHK-14 は CHK-05 に統合)*
