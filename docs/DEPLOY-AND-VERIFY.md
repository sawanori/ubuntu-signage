...
サイネージアプリを boot 時に自動起動するには、Pi4 の **デスクトップ自動ログイン** が必須です。
自動ログインなしの場合、XDG autostart は起動しません (`loginctl enable-linger` は
グラフィカルセッションを起動しないため代替になりません)。

```bash
# 実機で要実行: GUI 設定 (Pi OS 向け。Ubuntu の場合は「設定 → ユーザー → 自動ログイン」で設定)
sudo raspi-config
# System Options → Boot / Auto Login → Desktop Autologin (B4) を選択

# 非対話モード (スクリプト用・Pi OS 専用):
