...
| 項目 | 説明 | デフォルト |
|------|------|-----------|
| `siteUrl` | 下層に表示するサイネージサイトの URL | `http://localhost` |
| `videoFolderPath` | 広告動画 MP4 が置かれたフォルダの絶対パス | `""` (空) |
| `intervalMinutes` | 広告割り込み間隔 (1/5/10/15/30 分) | `10` |
...
# 注意: test:e2e スクリプトはまだ package.json に追加されていない
# 追加後は以下で実行する:
...
`src/main/utils/powerBlocker.ts` で管理されており、追加の設定は不要です。
...
| ネット断 | Wi-Fi オフ | `ERROR siteView.loadFailed` | バックオフ再試行・広告層は継続 |
